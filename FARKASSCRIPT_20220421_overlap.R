...
overlap.analysis = function ( f.df.o )
{
  ...
  write.table(summtable, str_c(overlap.dir, '/../overlap_summary.csv'), sep = ';', row.names = FALSE, col.names = TRUE, dec = ',')
  
  return (summtable)
}


overlap.analysis.bnpi = function ( f.df.en )
{
  f.df.o = f.df.en
  ...
  overlap.analysis( f.df.o )
}


overlap.analysis.cserkesz = function ( f.cs )
{
  overlap.analysis( f.cs )
}


overlap.analysis.full = function ( f.df.en, f.cs )
{
  f.df.o = f.df.en
  f.df.o$Felvetel.tartalma[f.df.o$Felvetel.tartalma == "hiker"] <- "Human disturbance"
  f.df.o$Felvetel.tartalma[f.df.o$Felvetel.tartalma == "rider"] <- "Human disturbance"
