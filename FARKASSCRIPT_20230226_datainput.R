...
read.bnp.farkas.data = function( farkasmappa = './', extended = FALSE, miscellaneous = FALSE, merge.time = NULL, hum.den = "none", data.months = NULL, covid = 'pre' )
{
  ...
  f.df <- f.df[f.df$Keszitette != "Pongrácz Ádám", ]
  ...
  {
    month.filter = sapply(f.df$Datum, function(y) as.numeric(str_split(as.character(y), '-')[[1]][2]) %in% data.months)
    f.df = f.df[month.filter == TRUE,]
  }
  ...
  helyszin.osszhuman = sapply(unique(f.df$Helyszin), function(x){length(which(f.df[which(f.df$Helyszin==x), "Felvetel.tartalma2"] == "emberi zavarás"))})
